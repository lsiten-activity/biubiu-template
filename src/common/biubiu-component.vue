...
<template>
  <div id="slider-view" class="slider-view" v-if="loaded">
    ...
    <div
      ...
      :id="`biubiu-render-id-_component_${index}`"
      :key="index"
...
import BiubiuComponentsLoader from './biubiu-remote-component-loader';

export default {
  name: 'biubiu-component',
  data() {
    return {
      init: false, // biubiu-admin 内嵌 iframe 初始化完成
      ...
      isEdit,
      components: window.__biubiu_config__.components.length
        ? window.__biubiu_config__.components // window.__biubiu_config__.components 是服务端注入的用户选择组件
      ...
      page: {
        ...
        props: (window.__biubiu_config__.pageData && window.__biubiu_config__.pageData.props) || config.pageConfig.data
      }
    };
  },
  created() {
